...
const STR = {
  ru:{brand:'US Team Fleet',title:'Drop / Hook',
    ...
    send:'Отправить',sending:'Отправка…',done:'Готово ✔',
  ...
  en:{brand:'US Team Fleet',title:'Drop / Hook',
    ...
    send:'Send',sending:'Sending…',done:'Done ✔',
...
export default function Page(){
  ...
  async function askGeo(){
    if(!navigator.geolocation){ setState({status:'error',message:STR[lang].needGeo}); return; }
    navigator.geolocation.getCurrentPosition(
      p=>{ setGeo({lat:p.coords.latitude, lon:p.coords.longitude}); if(state.status==='error') setState({status:'idle'}); },
      ()=>{ setGeo(null); setState({status:'error', message:STR[lang].needGeo}); },
  ...
  async function onSubmit(e:React.FormEvent<HTMLFormElement>){
    ...
    if(!geo){ setState({status:'error',message:t.needGeo}); return; }
    ...
    <div className="container">
      <div className="card">
        ...
        <form onSubmit={onSubmit}>
          ...
          {/* Локация */}
          <div className="geo">
            <button type="button" className="btn-geo" onClick={askGeo}>
              {geo ? (lang==='ru'?'Локация получена':'Location set') : (lang==='ru'?'Дать доступ к локации':'Allow location')}
            </button>
          </div>
          ...
          <button className={`btn-primary btn-full ${state.status==='done'?'btn-done':''}`} type="submit" disabled={state.status==='sending'}>
            {state.status==='done' ? (lang==='ru'?'Отправлено':'Sent') : state.status==='sending' ? t.sending : t.send}
          </button>
        </form>
